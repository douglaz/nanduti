...
impl FedimintClientWrapper {
    ...
    pub async fn new(invite_code: &str, data_dir: Option<&Path>) -> Result<Self> {
        ...
        let base_dir = data_dir
            ...
            .unwrap_or_else(|| PathBuf::from(".fedimint-nwc"));
    ...
    pub async fn fetch_gateways_with_vetted_status(
        &self,
    ) -> Result<Vec<(LightningGatewayAnnouncement, bool)>> {
        let gateways = self.fetch_gateways().await?;
        let vetted_ids = self.fetch_vetted_gateways().await.unwrap_or_default();

        let gateways_with_status: Vec<(LightningGatewayAnnouncement, bool)> = gateways
            .into_iter()
            .map(|g| {
                let is_vetted = vetted_ids.contains(&g.info.gateway_id.to_string());
                (g, is_vetted)
            })
            .collect();

        let vetted_count = gateways_with_status.iter().filter(|(_, v)| *v).count();
        info!(
            "Found {} gateways ({} vetted) in federation {}",
    ...
    pub async fn select_gateway(&self) -> Result<Option<LightningGateway>> {
        ...
        // Fetch vetted gateway IDs
        let vetted_gateway_ids = self.fetch_vetted_gateways().await.unwrap_or_default();

        // Separate gateways into vetted and non-vetted
        let (vetted, non_vetted): (Vec<_>, Vec<_>) = gateways
            .into_iter()
            .partition(|g| vetted_gateway_ids.contains(&g.info.gateway_id.to_string()));

        // Prefer vetted gateways if available
        let selected = if !vetted.is_empty() {
            info!(
        ...
        } else if !non_vetted.is_empty() {
            info!(
                "No vetted gateways available, selecting from {} non-vetted gateways in federation {}",
                non_vetted.len(),
                self.federation_name
            );
            non_vetted
        ...
        if let Some(ref gateway) = selected {
            let is_vetted = vetted_gateway_ids.contains(&gateway.gateway_id.to_string());
            info!(
                "Selected {} gateway {} for federation {}",
                if is_vetted { "vetted" } else { "non-vetted" },
                gateway.gateway_id,
                self.federation_name
            );
        }
    ...
    pub async fn fetch_vetted_gateways(&self) -> Result<Vec<String>> {
        // Try to get the meta module
        let meta_module = match self.client.get_first_module::<MetaClientModule>() {
            Ok(module) => module,
            Err(_) => {
                // Meta module not available, return empty list
                info!(
                    "Meta module not available in federation {}",
                    self.federation_name
                );
                return Ok(vec![]);
            }
        ...
        if let Some(consensus) = consensus_value {
            ...
            match consensus.value.to_json() {
                Ok(json) => {
                    ...
                    if let Some(vetted) = json.get("vetted_gateways") {
                        if let Some(gateway_array) = vetted.as_array() {
                            ...
                            info!(
                                "Found {} vetted gateways in federation {} (revision {})",
                                gateway_ids.len(),
                                self.federation_name,
                                consensus.revision
                            );
                ...
                Err(e) => {
                    info!(
                        "Failed to parse meta value as JSON in federation {}: {}",
                        self.federation_name, e
                    );
                }
            }
        }

        // No vetted gateways found
        info!(
            "No vetted gateways found in federation {}",
            self.federation_name
        );
        Ok(vec![])
    }
