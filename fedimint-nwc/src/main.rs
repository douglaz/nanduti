...
use fedimint_nwc_api::RoutingStrategy;
...
async fn serve(args: ServeArgs) -> Result<()> {
    ...
    tracing::info!("Starting Fedimint NWC server");

    // Create server config
    let config = fedimint_nwc_api::ServerConfig {
        host: args.host,
        port: args.port,
        relays: args.relays,
    ...
    // Start server
    fedimint_nwc_api::start_server(config).await?;
